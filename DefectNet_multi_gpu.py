...
tf.flags.DEFINE_string("logs_dir", "model/", "path to logs directory")
tf.flags.DEFINE_string("patch_dir", "/home/ubuntu/datasets/p0171-ml-dataset-blade/Patches200/Train/", "path to dataset")
tf.flags.DEFINE_string("result_dir", "/home/ubuntu/results/", "path to save predict result")
...
tf.flags.DEFINE_integer('num_gpus', "2","How many GPUs to use.")
...
def inference(image, keep_prob, reuse):
    """
...
def main(argv=None):
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        pred_annotation, logits = inference(image, keep_probability, reuse = tf.AUTO_REUSE)
        ...
        if FLAGS.mode == 'train':
            ...
            for i in xrange(FLAGS.num_gpus):
                with tf.device(assign_to_device('/gpu:{}'.format(i), ps_device='/cpu:0')):
                    ...
                    _y = annotation[i * FLAGS.batch_size: (i+1) * FLAGS.batch_size]
                    print(i+100)
                    pred_annotation, logits = inference(_x, keep_probability, reuse=reuse_vars)
        ...
        with tf.Session(config = config) as sess:
            ...
            if FLAGS.mode == "train":
                for itr in xrange(MAX_ITERATION):
                    ...
                    train_images, train_annotations = train_dataset_reader.next_batch(FLAGS.batch_size * FLAGS.num_gpus)
                    ...
                    if itr % 500 == 0:
                        valid_images, valid_annotations = validation_dataset_reader.next_batch(FLAGS.batch_size * FLAGS.num_gpus)
